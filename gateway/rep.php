...
$ren_count=$db ['TELCO_SUB']->getResults("SELECT HOUR(unsubscribedon) hour_num,COUNT(*) AS ren_count FROM subscription WHERE
DATE(unsubscribedon) ='".$today."' AND unsubscribedon >= '".$fromdate."' AND unsubscribedon <='".$todate."' GROUP BY hour_num ;");
foreach($ren_count as $val){
    $data[$val['hour_num']]['ren_count'] = $val['ren_count'];
}
// console(LOG_LEVEL_TRACE,"HELLOOOOOOOOO".var_export($ren_count,TRUE));

// REN REVENUE

$ren_revenue=$db ['TELCO_SUB']->getResults("SELECT HOUR(unsubscribedon) hour_num,COUNT(*) AS ren_revenue FROM subscription WHERE
DATE(unsubscribedon) ='".$today."' AND unsubscribedon >= '".$fromdate."' AND unsubscribedon <='".$todate."' GROUP BY hour_num ;");
foreach($ren_revenue as $val){
    $data[$val['hour_num']]['ren_revenue'] = $val['ren_revenue'];
}
