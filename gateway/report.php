...
define("DB_CONNECTIONS", "VODACOM_WAP,VODACOM_SUB");
...
$tothitcount = $db['VODACOM_WAP']->getResults("SELECT HOUR(transtime) AS hour_num,COUNT(msisdn) AS totalhitcount FROM hit_analysis 
...
$uniqhitcount = $db['VODACOM_WAP']->getResults("SELECT HOUR(transtime) AS hour_num,COUNT(DISTINCT(msisdn)) AS uniqhitcount FROM hit_analysis
WHERE  transtime>='" . $fromdate . "' AND transtime<='" . $todate . "' GROUP BY hour_num");
...
// BLANK HIT COUNT
$blankhitcount = $db['VODACOM_WAP']->getResults("SELECT HOUR(transtime) AS hour_num,COUNT(1) AS blankhitcount FROM hit_analysis_bulk 
WHERE msisdn=''  AND transtime>='".$fromdate."' AND transtime<='".$todate."' GROUP BY hour_num");
...
$multihitcount = $db['VODACOM_WAP']->getResults("SELECT HOUR(date_val) AS hour_num,COUNT(msisdn) AS multihitcount,COUNT(DISTINCT msisdn) AS blockedhitcount  FROM filtered_numbers 
...
$aaresult = $db['VODACOM_WAP']->getResults("SELECT HOUR(transtime) AS hour_num, COUNT(DISTINCT msisdn) AS activecnt, user_status FROM hit_analysis 
WHERE  user_status = 'ALREADY ACTIVE' and transtime>='".$fromdate."' AND transtime<='".$todate."' GROUP BY hour_num");
...
$dndresult = $db['VODACOM_WAP']->getResults("SELECT HOUR(transtime) AS hour_num, COUNT(DISTINCT msisdn) AS dndcnt, user_status FROM hit_analysis 
WHERE  user_status = 'DND' and transtime>='".$fromdate."' AND transtime<='".$todate."' GROUP BY hour_num");
...
$rtcgcnt = $db['VODACOM_WAP']->getResults("SELECT HOUR(transtime) AS hour_num, COUNT(*) AS rtcgcnt FROM subscription_detail 
WHERE  transtime>='" . $fromdate . "' AND transtime<='" . $todate . "' AND transactionid LIKE '%SUB%' GROUP BY hour_num");
...
$uniqrtcgcnt = $db['VODACOM_WAP']->getResults("SELECT HOUR(transtime) AS hour_num, COUNT(DISTINCT msisdn) AS uniqrtcgcnt FROM subscription_detail
WHERE  transtime>='" . $fromdate . "' AND transtime<='" . $todate . "' AND transactionid LIKE '%SUB%' 
 GROUP BY hour_num");
...
$agencysuccesscnt = $db['VODACOM_SUB']->getResults("SELECT HOUR(subscribedon) AS hour_num,DATE(subscribedon) AS DATE,SUM(IF(postback_status='success',1,0)) AS agencysuccesscnt FROM postback_history 
WHERE  subscribedon >= '".$fromdate."' AND subscribedon <= '".$todate."'  GROUP BY HOUR(subscribedon)");
...
$agencyholdcnt = $db['VODACOM_SUB']->getResults("SELECT HOUR(subscribedon) AS hour_num,DATE(subscribedon) AS DATE,SUM(IF(postback_status='hold',1,0)) AS agencyholdcnt 
FROM postback_history WHERE  subscribedon >= '".$fromdate."' AND subscribedon <= '".$todate."'  GROUP BY HOUR(subscribedon)");
...
$subsuccesscnt=$db ['VODACOM_SUB']->getResults("SELECT HOUR(subscribedon) hour_num,COUNT(*) AS subsuccesscnt FROM subscription WHERE 
 subscribedon >='".$fromdate."' AND subscribedon<='".$todate."'  AND status='active' GROUP BY hour_num ");
...
$subfailedcnt=$db ['VODACOM_SUB']->getResults("SELECT HOUR(subscribedon) hour_num,COUNT(*) AS subfailedcnt FROM subscription WHERE
 subscribedon >='".$fromdate."' AND subscribedon<='".$todate."'  AND status='failed' GROUP BY hour_num ");
...
$subpendingcnt=$db ['VODACOM_SUB']->getResults("SELECT HOUR(unsubscribedon) hour_num,COUNT(*) AS subpendingcnt FROM subscription WHERE
 subscribedon >='".$fromdate."' AND subscribedon<='".$todate."'  AND status='pending' GROUP BY hour_num ");
...
$deactcuscnt=$db ['VODACOM_SUB']->getResults("SELECT HOUR(unsubscribedon) hour_num,COUNT(*) deactcuscnt  FROM subscription 
WHERE unsubscribemode IN('wap') AND unsubscribedon >= '".$fromdate."'  AND unsubscribedon <= '".$todate."' GROUP BY hour_num");
...
$deactsdcnt=$db ['VODACOM_SUB']->getResults("SELECT HOUR(unsubscribedon) hour_num,COUNT(*) deactsdcnt FROM subscription 
WHERE DATE(subscribedon) ='".$today."' AND unsubscribedon >= '".$fromdate."' AND unsubscribedon <='".$todate."' AND unsubscribemode IN('wap') GROUP BY hour_num");
...
$varMessage = $varMessage . '<h3>VODACOM ZA Hourly Report - ' . $fromdate . ' To ' . date("Y-m-d H:i:s", strtotime("now")) . '</h3> 
	<i style="font-family: \'Source Sans Pro\',\'Helvetica Neue\',Helvetica,Arial,sans-serif; font-size: 13px;"> 
        ...
        <span style="color: red;">TH</span> - Total hits,
                 ...
                 <span style="color: red;">UNIQ RTCG</span> - Unique Redirect to CG,
	...
	<table style="border-collapse: collapse;" border="1" cellspacing="0" cellpadding="0" >	  
       ...
       <th style="font-family:sans-serif;font-size: 12px;padding: 5px;border: 1px solid #000; background-color: #f8c471; color:#000; text-align: center" colspan="2">CGR/SUB COUNT</th>
...
$ten_days=   $db['VODACOM_SUB']-> getResults("SELECT `date`,SUM(total_hits) AS `total_hit`,SUM(unique_hits) AS `uniq_hit`,SUM(blank_msisdn) AS `blank_hit`,
SUM(blocked_msisdn) AS `mhbc_cnt`,SUM(dnd_cnt) AS `dnd_cnt`,SUM(aa_cnt) AS `already_active`,SUM(rtcg_cnt) AS `rtcg_cnt`,
SUM(uniq_rtcg_cnt) AS `uniq_rtcg_cnt`,SUM(agnt_success) AS `ag_succ`,SUM(agnt_hold) AS `ag_hold`,SUM(sub_succ) AS `sub_success`,SUM(sub_fail) AS `sub_failed`,
SUM(sub_pend) AS `sub_pending`,SUM(cust_deact) AS `deact_cus`,SUM(sameday_deact) AS `deact_sd`,SUM(ren_succ) AS ren_succ,SUM(ren_revenue) AS ren_revenue
...
<table style="border-collapse: collapse;" border="1" cellspacing="0" cellpadding="0" >
        ...
        foreach($ten_days as $val){
            ...
            $data2[$val['date']]['sub_pending']=$val['sub_pending'];
            $data2[$val['date']]['deact_cus']=$val['deact_cus'];
            $data2[$val['date']]['deact_sd']=$val['deact_sd'];
            $data2[$val['date']]['ren_succ']=$val['ren_succ'];
...
foreach($date_range as $val){
    ...
    $ten_sub_pending=isset($data2[$val]["sub_pending"])?$data2[$val]["sub_pending"]:"0";
    $ten_deact_cus=isset($data2[$val]["deact_cus"])?$data2[$val]["deact_cus"]:"0";
    $ten_deact_sd=isset($data2[$val]["deact_sd"])?$data2[$val]["deact_sd"]:"0";
    $ten_ren_succ=isset($data2[$val]["ren_succ"])?$data2[$val]["ren_succ"]:"0";
...
$varMessage.='<tr>
        ...
        <td style="font-family:sans-serif;font-size: 12px;padding: 3px;"><p align="center">'.$tot_ten_sub_pending[]=$ten_sub_pending.'</td>
        <td style="font-family:sans-serif;font-size: 12px;padding: 3px;"><p align="center">'.$tot_ten_ren_succ[]=$ten_ren_succ.'</td>
    ...
    $varMessage.='<tr>
        ...
        <td style="font-family:sans-serif;font-size: 12px;padding: 5px;border: 1px solid #000; background-color: #f8c471; color: #000; text-align: center;"><p align="center"><strong>'.array_sum($tot_ten_sub_pending).'</strong></td>
        <td style="font-family:sans-serif;font-size: 12px;padding: 5px;border: 1px solid #000; background-color: #f8c471; color: #000; text-align: center;"><p align="center"><strong>'.array_sum($tot_ten_ren_succ).'</strong></td>
    ...
    $three_mons=$db['VODACOM_SUB']->getResults("SELECT DATE_FORMAT(`date`,'%Y-%m') AS mon_val,SUM(total_hits) AS `total_hit`,SUM(unique_hits) AS `uniq_hit`,
    SUM(blank_msisdn) AS `blank_hit`,SUM(blocked_msisdn) AS `mhbc_cnt`,SUM(dnd_cnt) AS `dnd_cnt`,SUM(aa_cnt) AS `already_active`,
    SUM(rtcg_cnt) AS `rtcg_cnt`,SUM(uniq_rtcg_cnt) AS `uniq_rtcg_cnt`,SUM(agnt_success) AS `ag_succ`,SUM(agnt_hold) AS `ag_hold`,
    SUM(sub_succ) AS `sub_success`,SUM(sub_fail) AS `sub_failed`,SUM(sub_pend) AS `sub_pending`,
    SUM(cust_deact) AS `deact_cus`,SUM(sameday_deact) AS `deact_sd`,SUM(ren_succ) AS `ren_succ`,ROUND(SUM(ren_revenue),0) AS `ren_revenue`
    FROM hourly_flat WHERE `date`>=(CURDATE() - INTERVAL 3 MONTH)  GROUP BY MONTH(`date`) ");

        
    // $three_month_base_1=$db['VODACOM_WAP']->getResults("SELECT DATE_FORMAT(`date`,'%Y-%m')as mon_val,avail_base as tot_base from hourly_flat where date='".$yesterday."'");
    // $three_month_base_2=$db['VODACOM_WAP']->getResults("SELECT DATE_FORMAT(`date`,'%Y-%m') as mon_val,avail_base as  tot_base from hourly_flat where date='".$lastmonth."'");
    // $three_month_base_3=$db['VODACOM_WAP']->getResults("SELECT DATE_FORMAT(`date`,'%Y-%m') as mon_val,avail_base as tot_base from hourly_flat where date='".$lastmonth_two."'");
    ...
    $varMessage = $varMessage.'<h3>VODACOM ZA HOURLY REPORT - Last 3 Months</h3>
        ...
        foreach($three_mons as $val){ 
            ...
            $data3[$val['mon_val']]['sub_pending']=$val['sub_pending'];
            $data3[$val['mon_val']]['deact_cus']=$val['deact_cus'];
            $data3[$val['mon_val']]['deact_sd']=$val['deact_sd'];
            $data3[$val['mon_val']]['ren_succ']=$val['ren_succ']; 
...
foreach($mon_range as $vals){
        ...
        $three_sub_pending=isset($data3[$vals]["sub_pending"])?$data3[$vals]["sub_pending"]:"0";
        ...
        $three_ren_succ=isset($data3[$vals]["ren_succ"])?$data3[$vals]["ren_succ"]:"0";
	...
	$varMessage.='<tr>
            ...
            <td style="font-family:sans-serif;font-size: 12px;padding: 3px;"><p align="center">'.$tot_three_sub_pending[]=$three_sub_pending.'</td>
            <td style="font-family:sans-serif;font-size: 12px;padding: 3px;"><p align="center">'.$tot_three_ren_succ[]=$three_ren_succ.'</td>
    ...
    $varMessage.='<tr>
		...
		<td style="font-family:sans-serif;font-size: 12px;padding: 5px;border: 1px solid #000; background-color: #f8c471; color: #000; text-align: center;"><p align="center"><strong>'.array_sum($tot_three_sub_pending).'</strong></td>
        <td style="font-family:sans-serif;font-size: 12px;padding: 5px;border: 1px solid #000; background-color: #f8c471; color: #000; text-align: center;"><p align="center"><strong>'.array_sum($tot_three_ren_succ).'</strong></td>
